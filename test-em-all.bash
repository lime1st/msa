...
function testCircuitBreaker() {
    ...
    assertEqual "CLOSED" "$(docker-compose exec -T composite curl -s http://composite:8080/actuator/health | jq -r .components.circuitBreakers.details.product.details.state)"
    ...
    assertEqual "OPEN" "$(docker-compose exec -T composite curl -s http://composite:8080/actuator/health | jq -r .components.circuitBreakers.details.product.details.state)"
    ...
    assertEqual "HALF_OPEN" "$(docker-compose exec -T composite curl -s http://composite:8080/actuator/health | jq -r .components.circuitBreakers.details.product.details.state)"
    ...
    assertEqual "CLOSED_TO_OPEN"      "$(docker-compose exec -T composite curl -s http://composite:8080/actuator/circuitbreakerevents/product/STATE_TRANSITION | jq -r .circuitBreakerEvents[-3].stateTransition)"
    assertEqual "OPEN_TO_HALF_OPEN"   "$(docker-compose exec -T composite curl -s http://composite:8080/actuator/circuitbreakerevents/product/STATE_TRANSITION | jq -r .circuitBreakerEvents[-2].stateTransition)"
    assertEqual "HALF_OPEN_TO_CLOSED" "$(docker-compose exec -T composite curl -s http://composite:8080/actuator/circuitbreakerevents/product/STATE_TRANSITION | jq -r .circuitBreakerEvents[-1].stateTransition)"
}
...
ACCESS_TOKEN=$(curl -k https://writer:writer@$HOST:$PORT/oauth2/token -d "grant_type=client_credentials&scope=product:write product:read" -s | jq .access_token -r)
...
READER_ACCESS_TOKEN=$(curl -k https://reader:reader@$HOST:$PORT/oauth2/token -d "grant_type=client_credentials&scope=product:read" -s | jq .access_token -r)
